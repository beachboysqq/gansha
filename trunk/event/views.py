...
from event.models import Event,Sub_event,History,Concern
...
def event(request):
    ...
    se_list = Sub_event.gql("where event = :1 order by start_date,end_date",event) 
...
def add_sub_event(request):
    ...
    if request.method == 'POST':
        se = Sub_event(user=user,event=pe,content=request.POST['content'])
...
def del_sub_event(request):
    ...
    if pe.start_date == se.start_date:
        pe.start_date = Sub_event.all().order('start_date').get().start_date
    if pe.end_date == se.end_date:
        pe.end_date = Sub_event.all().order('-end_date').get().end_date
    pe.put()
    se.delete()
