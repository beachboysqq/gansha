...
def home(request,whose):
    ...
    # record basic info for user into session
        ...
        se_list = Sub_event.all().filter('user =',host).filter(
            'is_done =',False ).filter( 'start_date <',datetime.date.today() )
        ...
        # get recent comments
        comments = Comment.all().filter('user_id =',host).order('-publish_time').fetch(5)
        # get recent messages
        mes_li = Mes.all().filter('receiver =',user).order('-publish_time').fetch(5)
